...
#define PITCH_ABSOLUTE_ZERO 2128
#define ENCODER_TO_PALES_RATIO (360.0f / 240.0f) // 30 dents pour l'encodeur versus 20 dents pour les pales
#define PALE_RADIUS 0.9135f //C13
...
float CalcPitchAngle_raw_to_deg() {
	...
	//1 tour d'encodeur (360) égale 240 degrée pour un tour de pale : rotor C12 avant PFE H2025
...
#define ALGO_C12 1
	...
	if (ALGO_C12) {
		...
		if (tsr <= 2.5) {
			...
			pitch_target = -440.017f + 435.068f * tsr + 196.501f * tsr2 - 487.301f * tsr3
					+ 324.152f * tsr4 - 119.048f * tsr5 + 27.5701f * tsr6
					- 4.19495f * tsr7 + 0.419413f * tsr8 - 0.0265633f * tsr9
					+ 0.000967116 * tsr10 - 0.000015428 * tsr11;
		}
...
/*
 ...
 float pitch_encoder_centered = 0;
 if (PITCH_ABSOLUTE_ZERO <= HALF_MAX_PITCH_VALUE_RAW) { //0 à 2047
 if (sensor_data.pitch_encoder
 < (MAX_PITCH_VALUE_RAW - (HALF_MAX_PITCH_VALUE_RAW - PITCH_ABSOLUTE_ZERO))) {
 pitch_encoder_centered = (float) ((float) sensor_data.pitch_encoder
 + (float) HALF_MAX_PITCH_VALUE_RAW - (float) PITCH_ABSOLUTE_ZERO);
 } else {
 pitch_encoder_centered = (float) ((float) sensor_data.pitch_encoder
 + (float) HALF_MAX_PITCH_VALUE_RAW - (float) PITCH_ABSOLUTE_ZERO
 - (float) MAX_PITCH_VALUE_RAW);
 }
 } else if (PITCH_ABSOLUTE_ZERO > HALF_MAX_PITCH_VALUE_RAW) { //2048 à 4097
 if (sensor_data.pitch_encoder
 > (float) (PITCH_ABSOLUTE_ZERO - HALF_MAX_PITCH_VALUE_RAW)) {
 pitch_encoder_centered = (float) ((float) sensor_data.pitch_encoder
 - (float) ((float) PITCH_ABSOLUTE_ZERO
 - (float) HALF_MAX_PITCH_VALUE_RAW));
 } else {
 pitch_encoder_centered = (float) ((float) sensor_data.pitch_encoder
 + (float) MAX_PITCH_VALUE_RAW
 - (float) ((float) PITCH_ABSOLUTE_ZERO
 ...
 float pitch_angle = (float) pitch_encoder_centered
 * (float) ABSOLUTE_ENCODER_RESOLUTION_ANGLE_12BITS; //si 2047 = 180
 pitch_angle = pitch_angle / (float) ENCODER_TO_PALES_RATIO;
